...
watercolor.prototype = {
    ...
    stroke: function(b, a) {
				...
				for (i = 0; i < randomIntegerBetween(30, 40); i++) {
					this.context.fillStyle = "rgba(" + COLOR[0] + ", " + COLOR[1] + ", " + COLOR[2] + "," + Math.random()/12 + ")";
					x = x + randomIntegerBetween(-(brushSize/4), +(brushSize/4));
					y = y + randomIntegerBetween(-(brushSize/4), +(brushSize/4));
					rad = randomIntegerBetween(brushSize, brushSize*2);
					this.context.beginPath();
					this.context.arc(x, y, rad/2, 0, 2 * Math.PI);
					this.context.fill();
				}
